...
target= np.load("/Users/louitech_zero/VoidGaze/Data/results/target_salnet.npy") #target
std = np.load("/Users/louitech_zero/VoidGaze/Data/results/std_salnet.npy") #gaussstd
mean = np.load("/Users/louitech_zero/VoidGaze/Data/results/mean_salnet.npy") # gaussmean
...
def update(frame):
    ...
    stdx = max(std[frame,0]*3840,0)     #std[frame]*3840
    stdy = max(std[frame,1]*2048,0)    #max(std[frame,1]*2048,diffy*2048) # stdx
    Tx.append(tx)
    Ty.append(ty)
    Mx.append(mx)
    My.append(my)
...
ani = FuncAnimation(fig, update, frames=np.linspace(2500, len(target)-1, len(target)),
